...
static Media_Listen_Func* play_stop = NULL;
...
void media_Set_Mute(gboolean m_mute) {
...
void set_Recorder_Listener(Media_Listen_Func func) {
	if (NULL == play_stop) {
		play_stop = (Media_Listen_Func*) malloc(sizeof(Media_Listen_Func));
	}
	if (NULL == func)	{
		free(play_stop);
	}
